...
async def file_send(interaction: discord.ui.text_input, endpoint: str, index: str, url:bool = False):
    ...
    data = json.loads(requests.get(f'https://www.dnd5eapi.co/api/{endpoint if index == '' else f"{endpoint}/{index}"}').text)
    data = json.dumps(data['results'] if endpoint == 'list' else data,indent=2)
